...
class ProcessInstanceFactory {
	...
	def static createTaskstatusInstance(Queue parent, Taskstatus ts){
		...
		Queue msg = new Queue(type: 'Taskstatus', finished: false, description: ts.task.description, idprocess: ts.task.process.id, ord: 0, user: user, task: ts.task, status: ts.status,
			startdate: startdt, signaldate: signaldt, enddate: enddt, parent: parent, taskstatus: ts, maxrepeat: ts.maxrepeat, initstatus: ts.status).save(failOnError: true)
	}
}
